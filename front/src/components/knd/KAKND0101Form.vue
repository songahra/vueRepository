  ...
  <div id="ct">
    <section class="card">
      <header class="card-header">
        <h2 class="card-title">
          <span class="i-rounded bg-danger"><i class="icon-arrange" /></span>지식자료실
        </h2>
        <div
          v-if="chkUserType()"
          class="btn-container"
    ...
    this.columnDefs = [
      ...
      { headerName: '작성자', field: 'writer', sortable: false, filter: true },
      {
        ...
        cellRenderer: function (params) { // 첨부파일 아이콘 ( 첨부파일 수 ) 표시
          console.log('params', params.data.file)
          return '<span><i class="icon-link"></i>(' + (params.data.file) + ')</span>'
        }
      },
