...
$(document).ready(function () {
  ...
  $('body').keydown(function () {
    if (event.keyCode === 122 && elem.msRequestFullscreen) event.preventDefault()
  })
})
...
function initDroppable (componentDiv, a) {
  $(componentDiv).find('.componentDroppable').droppable({
    ...
    drop: function (event, ui) {
      ...
      if ($(this).data('type') === 'left' || $(this).data('type') === 'right') { // 가로드롭
        ...
        if (!appendDirection || appendDirection === 'horizontal') {
          ...
          if ($(this).data('type') === 'left') $(this).parent().before(componentDiv)
          ...
          // append
          if ($(this).data('type') === 'left') componentParentDiv.append(componentDiv).append(cloneElement)
        ...
        const componentDiv = initNewComponent(title, bodyBg)

        if (!appendDirection || appendDirection === 'vertical') {
          ...
          if ($(this).data('type') === 'top') $(this).parent().before(componentDiv)
          else $(this).parent().after(componentDiv)
          ...
          const componentParentDiv = $('<div/>').addClass('component-vertical').css({
            width: $(this).parent().data('width'),
            height: $(this).parent().data('height')
          })
          ...
          // cloneElement
          const cloneElement = $(this).parent().clone()
          cloneElement.find('.ui-droppable-active').removeClass('ui-droppable-active')

          const componentDroppableList = cloneElement.children('.componentDroppable')

          for (let i = 0; i < componentDroppableList.length; i++) {
            $(componentDroppableList[i]).data('type', $($(this).parent().children('.componentDroppable')[i]).data('type'))
          }

          // append
          if ($(this).data('type') === 'top') componentParentDiv.append(componentDiv).append(cloneElement)
          else componentParentDiv.append(cloneElement).append(componentDiv)
...
function initBeforeResizable (componentDiv) {
  ...
  if ((axis === 'n' && appendDirection === 'vertical') || (axis === 'w' && appendDirection === 'horizontal')) {
    isAbleResize = true
  }
...
function initDelete (componentDiv) {
  ...
  $(componentDiv).find('.componentDelete').on('click', function () {
    ...
    if ($root.attr('id') === 'root' && (childrenList.length) === 1) {
      ...
      if ($root.attr('id') !== 'root' && (childrenList.length) === 2) {
      ...
      var prop = (appendDirection === 'horizontal') ? 'width' : 'height'
